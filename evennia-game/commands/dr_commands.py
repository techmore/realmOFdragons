...
from world.dr_combat import advance, aim, appraise_enemy, bash, block, combat_status, defend, dodge, feint, flee, guard, health_text, hurl, jab, kick, loot_corpse, maneuver_guide, parry, range_status, respawn_room_enemies, rest, retreat, revive, room_enemy_ids, scan_room, shoot, skin_corpse, slice_attack, stance, target_enemy, wait_recover
...
from world.dr_progression import advance_circle, circle_status, experience_summary, guild_ability_summary, guild_history_summary, guild_path_summary, guild_plan_summary, guild_title, guild_title_ladder, study_room, train_skill, unlocked_guild_perks, use_guild_boon, use_guild_capstone, use_guild_challenge, use_guild_drill, use_guild_focus, use_guild_lesson, use_guild_mastery, use_guild_mentor, use_guild_milestone, use_guild_passive, use_guild_perk, use_guild_practice, use_guild_rite, use_guild_signature, use_guild_technique
...
CHARACTER_HELP_TEXT = "\n".join(
    [
        ...
        "Guilds/Circles: guilds, registrar, join guild, guild/perks, guild history, title, experience, abilities, guild path, guild plan, mentor, lesson, perk, signature, milestone, focus, technique, passive, drill, practice, rite, challenge, boon, capstone, mastery, study, train, circle, circle status.",
        ...
        "Combat: scan, target <enemy>, appraise target, maneuvers/combat tactics, range, advance, retreat, combat, stance, aim, shoot/fire, hurl/throw, feint, jab/attack, slice/cut, kick, bash, defend, guard/brace, dodge, parry, block, flee, wait/recover, rest, skin corpse, loot corpse, revive/stand.",
...
CHARACTER_HELP_TOPICS = {
    ...
    "combat": "\n".join(
        [
            ...
            "maneuvers / combat tactics - show range-appropriate attacks, setup moves, defenses, equipment hints, and current set states.",
            ...
            "feint - prepare the next melee strike. jab / attack - quick melee maneuver using Small Edged and agility. slice / cut - held-blade melee maneuver using Small Edged and reflex.",
    ...
    "progression": "\n".join(
        [
            ...
            "4. Train and circle: use guild path, guild plan, title, experience, train, study, mentor, lesson, perk, signature, milestone, passive, drill, practice, rite, challenge, boon, capstone, mastery, skills, circle status, circle, abilities, focus, and technique at your own guild registrar through Circle 10.",
...
class CmdDRGuildPlan(Command):
    """
    Show the full supported Circle 1-10 guild plan.
...
class CmdDRGuildLesson(Command):
    """
    Take a Circle-scaled guild lesson at your registrar.
    ...
    def func(self):
        ...
        events = use_guild_lesson(state)
        character.db.skills = state["skills"]
        character.msg("\n".join(events))


class CmdDRGuildPerkAction(Command):
    """
    Invoke the current earned Circle perk at your registrar.
    ...
    def func(self):
        ...
        events = use_guild_perk(state)
        character.db.skills = state["skills"]
        character.msg("\n".join(events))


class CmdDRGuildSignature(Command):
    """
    Use the guild's always-available signature action.
...
class CmdDRGuildChallenge(Command):
    """
    Attempt a Circle 7+ guild challenge at your own registrar.
    ...
    key = "challenge"
    aliases = ["guild challenge", "trial", "guild trial"]
    locks = "cmd:all()"
    help_category = "Dragon Realms"
    ...
    def func(self):
        ...
        events = use_guild_challenge(state)
        character.db.skills = state["skills"]
        character.msg("\n".join(events))
...
class CmdDRGuildMastery(Command):
    """
    Practice repeatable Circle 10 guild mastery at your own registrar.
    ...
    def func(self):
        ...
        events = use_guild_mastery(state)
        character.db.skills = state["skills"]
        character.msg("\n".join(events))
...
class CmdDRTalk(Command):
    """
    Talk to the current room's shopkeeper.
    ...
    key = "talk"
    aliases = ["ask", "talk shopkeeper", "ask shopkeeper"]
    locks = "cmd:all()"
    help_category = "Dragon Realms"

    def func(self):
        self.caller.msg(talk_shopkeeper(self.caller.location, self.args))
...
class CmdDRManeuvers(Command):
    """
    Show range-appropriate combat maneuver guidance.
    ...
    key = "maneuvers"
    aliases = ["maneuver", "combat tactics", "tactics guide", "moves"]
    locks = "cmd:all()"
    help_category = "Dragon Realms"

    def func(self):
        self.caller.msg(maneuver_guide(self.caller))
...
class CmdDRAim(Command):
    """
    Aim at a targeted enemy before a ranged attack.
    ...
    def func(self):
        self.caller.msg(aim(self.caller))
...
class CmdDRSlice(Command):
    """
    Slice a targeted enemy with a held small blade at melee range.
    ...
    def func(self):
        self.caller.msg(slice_attack(self.caller))
...
class CmdDRKick(Command):
    """
    Kick a targeted enemy at melee range.
    ...
    def func(self):
        self.caller.msg(kick(self.caller))
...
class CmdDRShoot(Command):
    """
    Shoot a targeted enemy at missile or pole range.
    ...
    def func(self):
        self.caller.msg(shoot(self.caller))


class CmdDRFeint(Command):
    """
    Feint at melee range to strengthen the next melee strike.
    ...
    def func(self):
        self.caller.msg(feint(self.caller))
...
class CmdDRDodge(Command):
    """
    Prepare to dodge the next close enemy pressure.
    ...
    def func(self):
        self.caller.msg(dodge(self.caller))


class CmdDRParry(Command):
    """
    Prepare to parry the next close enemy pressure with a held weapon.
    ...
    key = "parry"
    aliases = ["block weapon"]
    locks = "cmd:all()"
    help_category = "Dragon Realms"

    def func(self):
        self.caller.msg(parry(self.caller))


class CmdDRBlock(Command):
    """
    Prepare to block the next close enemy pressure with a shield.
    ...
    def func(self):
        self.caller.msg(block(self.caller))
