...
class DRDataTests(SimpleTestCase):
    ...
    def test_all_guilds_have_named_circle_one_to_ten_milestones(self):
        ...
        self.assertEqual(set(GUILD_MASTERIES), set(GUILDS))
        for guild_id, perk_names in GUILD_CIRCLE_PERK_NAMES.items():
            ...
            self.assertIn(GUILD_MASTERIES[guild_id]["skill"], build_starter_skills())
            self.assertTrue(GUILD_MASTERIES[guild_id]["name"])
            self.assertTrue(GUILD_MASTERIES[guild_id]["text"])
...
class DRCommandSmokeTests(TestCase):
    ...
    def test_focused_text_help_topics_cover_movement_and_combat(self):
        ...
        self.assertIn("lesson", CHARACTER_HELP_TOPICS["progression"])
    ...
    def test_all_guilds_join_and_reach_circle_ten_through_commands(self):
        ...
        for guild_id, guild_name in GUILDS.items():
            ...
            self.assertIn("lesson", post_join_registrar_text)
            ...
            self.assertIn("mastery", post_join_registrar_text)
            ...
            self.assertIn("Registrar lesson:", ability_text)
            self.assertIn("Use `lesson`", ability_text)
            ...
            self.assertIn("Circle 10 mastery:", ability_text)
            ...
            lesson_skill_id = GUILD_LESSONS[guild_id]["skill"]
            lesson_before = (character.db.skills[lesson_skill_id]["rank"] * 5) + character.db.skills[lesson_skill_id]["pool"]
            lesson_text = "\n".join(
                use_guild_lesson(
                    {
                        "guild_id": character.db.guild_id,
            ...
            self.assertIn(GUILD_LESSONS[guild_id]["name"], lesson_text)
            self.assertIn("Circle 10", lesson_text)
            lesson_after = (character.db.skills[lesson_skill_id]["rank"] * 5) + character.db.skills[lesson_skill_id]["pool"]
            self.assertGreater(lesson_after, lesson_before)
            character.execute_cmd("lesson")
            character.execute_cmd("guild lesson")
            lesson_command_after = (character.db.skills[lesson_skill_id]["rank"] * 5) + character.db.skills[lesson_skill_id]["pool"]
            self.assertGreater(lesson_command_after, lesson_after)
            reloaded_lesson = ObjectDB.objects.get(id=character.id)
            reloaded_lesson_after = (reloaded_lesson.db.skills[lesson_skill_id]["rank"] * 5) + reloaded_lesson.db.skills[lesson_skill_id]["pool"]
            self.assertGreaterEqual(reloaded_lesson_after, lesson_command_after)
            ...
            self.assertIn(GUILD_MASTERIES[guild_id]["name"], mastery_text)
            self.assertIn("Circle 10", mastery_text)
            mastery_after = (character.db.skills[mastery_skill_id]["rank"] * 5) + character.db.skills[mastery_skill_id]["pool"]
            self.assertGreater(mastery_after, mastery_before)
            character.execute_cmd("mastery")
            character.execute_cmd("guild mastery")
            mastery_command_after = (character.db.skills[mastery_skill_id]["rank"] * 5) + character.db.skills[mastery_skill_id]["pool"]
            self.assertGreater(mastery_command_after, mastery_after)
            reloaded_mastery = ObjectDB.objects.get(id=character.id)
            reloaded_mastery_after = (reloaded_mastery.db.skills[mastery_skill_id]["rank"] * 5) + reloaded_mastery.db.skills[mastery_skill_id]["pool"]
            self.assertGreaterEqual(reloaded_mastery_after, mastery_command_after)
            circle_ten_path = "\n".join(
                guild_path_summary(
                    {
                        "guild_id": character.db.guild_id,
    ...
    def test_circle_status_guides_unaffiliated_and_ready_characters(self):
        ...
        unaffiliated_lesson = use_guild_lesson(
            {
                "guild_id": character.db.guild_id,
                "circle": character.db.circle,
                "skills": character.db.skills,
    ...
    def test_scan_target_advance_range_and_retreat_commands(self):
        ...
        self.assertIn("shoot / fire", missile_guide)
        ...
        self.assertIn("shoot / fire", pole_guide)
    ...
    def test_shoot_spends_arrows_and_trains_bows_and_missile_mastery(self):
        character = self.make_character("Shoot Smoke")
        self.walk_to_room(character, "crossing-RV02-002")

        self.assertIn("practice_arrows", character.location.db.shop_stock)
        character.execute_cmd("buy practice_arrows")
        self.assertIn("practice_arrows", character.db.inventory)
        character.execute_cmd("target rv-wolf-cub")
        aim_text = aim(character)
        self.assertIn("hurl or shot", aim_text)
        character.execute_cmd("wait")
        shoot_text = shoot(character)
        self.assertIn("You shoot Wolf Cub", shoot_text)
        self.assertIn("careful aim adds force", shoot_text)
        self.assertIn("practice_arrows", shoot_text)
        self.assertIn("too far away to press back", shoot_text)
        self.assertNotIn("practice_arrows", character.db.inventory)
        self.assertFalse(
            [
                obj
                for obj in character.contents
                if obj.db.object_type == "item" and obj.db.item_id == "practice_arrows"
            ]
        )
        self.assertGreater(character.db.skills["bows"]["pool"], 0)
        self.assertGreater(character.db.skills["missile_mastery"]["pool"], 0)
        self.assertFalse(character.db.engagement["aimed"])
        character.execute_cmd("fire")
        character.execute_cmd("wait")
        self.assertIn("need practice_arrows", shoot(character))
        character.execute_cmd("advance")
        character.execute_cmd("advance")
