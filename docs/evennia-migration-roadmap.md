...
6. `Combat`
   ...
   - Current status: initial Crossing enemy deployments build real enemy NPC objects, and command smoke covers `scan`, `target`, `range`, `advance`, `retreat`, `stance`, deterministic `jab`/`attack`, roundtime blocking, manual `wait`/`recover`, damage, health, stance-modified enemy retaliation, defeat loot, manual respawn, balance mutation, and enemy defeat cleanup.
   - Current next blocker: no automatic respawn, real timed recovery, object drops/corpses, or asynchronous Scripts exist yet.
