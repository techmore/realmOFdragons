...
6. `Combat`
   ...
   - Current status: initial Crossing enemy deployments build real enemy NPC objects, and command smoke covers `scan`, `target`, `range`, `advance`, `retreat`, `stance`, deterministic `jab`/`attack`, `bash`, `defend`, `flee`, roundtime blocking, manual `wait`/`recover`, damage, health, stance-modified enemy retaliation, defeat loot, manual respawn, respawn Script hooks, recovery Script hooks, automatic combat pressure Script lifecycle, balance mutation, and enemy defeat cleanup.
   - Current next blocker: no object drops/corpses or deeper asynchronous combat Scripts exist yet.
