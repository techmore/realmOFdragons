...
- Partially implemented after this audit: Evennia now builds in-room enemy NPC objects for the current Crossing hunting rooms and supports `scan`, `target`, `range`, `advance`, `retreat`, `stance`, deterministic `jab`/`attack`, roundtime blocking, manual `wait`/`recover`, damage, health, stance-modified enemy retaliation, defeat loot, balance mutation, and enemy defeat cleanup.
- Remaining work: add more maneuvers, enemy recovery/respawn, real timed recovery, object drops/corpses, and Evennia Script/ticker-driven asynchronous combat behavior.
